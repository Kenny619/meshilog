...
export async function manualUpdateShops(env: CloudflareBindings) {
	...
	const prms = targets.map((target) => crawlRestaurant(target, env));
	...
	await putKV(env, "SHOPS", updatedSHOPS);
	...
	return `[get-shopDetail] Success: ${results.length} shops updated`;
}
...
async function crawlRestaurant(
	...
	const rewriter = new HTMLRewriter()
		...
		.on("#js-detail-score-open > p > b > span", {
			text({ text }) {
				score += text;
			},
		})
		.on("span.rdheader-rating__review-target em.num", {
			text({ text }) {
				reviews += text;
			},
		})
